        ...
        if(token){
            if(isPremiumUser === 'true'){
                ...
                downloadReport.addEventListener('click',async()=>{
                    ...
                    await axios.get("http://54.236.200.99:3000/expence/download",{headers : {'Auth' : token}}).then((response)=>{
                ...
                showLeaderboard.addEventListener('click', async()=>{
                    ...
                    const res = await axios.get("http://54.236.200.99:3000/premium/showLeaderboard",{headers : {'Auth' : token}});
                ...
                buyPremium.addEventListener('click',async (e)=>{
                    const res = await axios.get("http://54.236.200.99:3000/purchase/primiumMembership",{headers : {'Auth' : token}});
                    var options = {
                        "key" : res.data.key_id,
                        "order_id" : res.data.order.id,
                        "handler" : async function (responce) {
                            await axios.post("http://54.236.200.99:3000/purchase/updateStatus",{
            ...
            submit.addEventListener('click',async ()=>{
                ...
                try{
                    const res = await axios.post("http://54.236.200.99:3000/expence/addExpence",obj);
                    display(1);
                }
            ...
            async function display(page){
                ...
                if(rowsPerPage>=5){
                    res = await axios.get(`http://54.236.200.99:3000/expence/getExpence?page=${page}&limit=${rowsPerPage}`, { headers : {'Auth' : token}});
                }else{
                    res = await axios.get(`http://54.236.200.99:3000/expence/getExpence?page=${page}&limit=10`, { headers : {'Auth' : token}});
                }
                tbody.innerHTML='';
                showbtn.innerHTML='';
                previous.innerHTML='';
                    ...
                    res.data.data.forEach(data => {
                        ...
                        if(data.type === 'income'){
                            ...
                            deleteBtn.addEventListener('click',async ()=>{
                                try{
                                    await axios.delete(`http://54.236.200.99:3000/expence/deleteExpence/${data.id}`);
                                    totalIncome.textContent = Number(totalIncome.textContent)-Number(data.amount);
